...
contract Voting {
   ...
   */
  constructor(uint tokens, uint pricePerToken, bytes32[] memory candidateNames) {
  ...
  // Note:
  // Data location is set as memory in a personal attempt to prevent writing into storage.
  // To my understanding, This actually requires copying data from storage to memory. (more gas)
  // Also the method has 'pure' modifier which 'Disallows modification or access of state'.
